...
test.describe.skip('Class Instantiation Tests', () => {
  ...
  test('No setTint errors should occur in console', async ({ page }) => {
    ...
    if (consoleErrors.length > 0) {
      ...
      throw new Error(
        `Found ${consoleErrors.length} setTint errors in console:\n${consoleErrors.join('\n')}`
      );
    }
...
test.describe.skip('Main Game Tests', () => {
  ...
  test('Game should load without setTint errors', async ({ page }) => {
    ...
    const setTintErrors = consoleErrors.filter(
      (error) =>
        (error.includes('setTint') || error.includes('clearTint')) &&
  ...
  test('Player and Enemy updates should not throw setTint/clearTint errors', async ({ page }) => {
    ...
    page.on('console', (msg) => {
      if (
        msg.type() === 'error' &&
        (msg.text().includes('setTint') || msg.text().includes('clearTint'))
      ) {
    ...
    if (setTintErrors.length > 0) {
      ...
      throw new Error(
        `Found ${setTintErrors.length} setTint/clearTint errors during gameplay:\n${setTintErrors.join('\n')}`
      );
    }
