...
test.describe('VibeCoder ally spawning (R3.x)', () => {
  ...
  // -----------------------------------------------------------------------
  test('R3.1: VibeSpawn is a global class with required constructor signature', async ({
    page,
  }) => {
  ...
  // -----------------------------------------------------------------------
  test('R3.2: chicken body velocity X sign matches direction to nearest enemy', async ({
    page,
  }) => {
  ...
  test('R3.3: doghouse emits a dog after > 3000ms', async ({ page }) => {
    ...
    const result = await page.evaluate(() => {
      ...
      const countBefore = player.vibeSpawns.filter(
        (s) => s && s.alive && s.type !== 'doghouse'
      ).length;

      // Simulate 3001ms of update ticks
      doghouse.update(3001);

      const countAfter = player.vibeSpawns.filter(
        (s) => s && s.alive && s.type !== 'doghouse'
      ).length;
