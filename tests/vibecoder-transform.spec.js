...
test.describe('VibeCoder costume + robot<->computer transform', () => {
  ...
  test('R2.4b: computer form blocks jump input (velocity.y and sprite.y unchanged)', async ({
    page,
  }) => {
    await equipVibeCoder(page);
    await startGameScene(page);
  ...
  test('R2.6: computer form visuals contain rectangle game objects only (no sprites/images)', async ({
    page,
  }) => {
    await equipVibeCoder(page);
    await startGameScene(page);
    ...
    const visualCheck = await page.evaluate(() => {
      ...
      const hasRectangles = visuals.some(
        (v) =>
          v &&
          v.type &&
  ...
  test('R2.7: vibeCoder key is visible in dragonCostumes keys (CraftScene-agnostic)', async ({
    page,
  }) => {
