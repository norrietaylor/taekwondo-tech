...
class GameScene extends Phaser.Scene {
  ...
  create() {
    try {
      ...
      this.add
        .text(512, 288, 'Game Error: ' + error.message, {
          fontSize: '24px',
          fill: '#ff0000',
    ...
    for (let i = 0; i < 20; i++) {
      ...
      switch (this.currentLevel) {
        ...
        case 6: // Earth level - floating rocks and leaves
          if (Math.random() > 0.5) {
            // Floating rock
            element = this.add.polygon(
              x,
              y,
              [-size / 2, 0, 0, -size / 2, size / 2, 0, size / 3, size / 2, -size / 3, size / 2],
              theme.accent,
              0.7
            );
          } else {
  ...
  createUI() {
    ...
    this.robotPartsText = this.add
      .text(20, 60, 'Parts: 0/5', {
        fontSize: '18px',
        fill: '#ffffff',
        backgroundColor: '#000000',
        padding: { x: 10, y: 5 },
    ...
    this.levelText = this.add
      .text(20, 140, `Level ${this.currentLevel}`, {
        fontSize: '18px',
        fill: '#ffffff',
        backgroundColor: '#000000',
        padding: { x: 10, y: 5 },
    ...
    this.finishLineIndicator = this.add
      .text(20, 180, '🏁 Reach the finish line!', {
        fontSize: '16px',
        fill: '#FFD700',
        backgroundColor: '#000000',
        padding: { x: 10, y: 5 },
  ...
  createHealthBar() {
    ...
    this.healthBarFill = this.add
      .rectangle(x + 2, y + 2, barWidth - 4, barHeight - 4, 0x00ff00)
      .setScrollFactor(0)
      .setDepth(101)
      .setOrigin(0, 0);
    ...
    this.healthText = this.add
      .text(x + barWidth / 2, y + barHeight / 2, '100/100', {
        fontSize: '14px',
        fill: '#ffffff',
      })
      .setScrollFactor(0)
  ...
  createFinishLine() {
    ...
    this.finishLineZone = this.add.rectangle(
      finishX,
      collisionZoneY,
      80,
      collisionZoneHeight,
  ...
  showCompletionChoice() {
    ...
    const overlay = this.add
      .rectangle(this.cameras.main.centerX, this.cameras.main.centerY, 400, 200, 0x000000, 0.8)
      .setOrigin(0.5)
      .setScrollFactor(0)
      .setDepth(300);
    ...
    const completeButton = this.add
      .text(this.cameras.main.centerX - 80, this.cameras.main.centerY + 50, 'Complete Now', {
        fontSize: '18px',
        fill: '#ffffff',
    ...
    const continueButton = this.add
      .text(this.cameras.main.centerX + 80, this.cameras.main.centerY + 50, 'Keep Playing', {
        fontSize: '18px',
        fill: '#ffffff',
    ...
    completeButton.on('pointerdown', () => {
      ...
      this.completeLevel();
      // Clean up UI
      [overlay, titleText, instructionText, completeButton, continueButton].forEach((obj) =>
        obj.destroy()
      );
    });
  ...
  calculateStarRating() {
    ...
    if (
      partsPercent >= 0.6 && // 60% of parts (minimum for meaningful progress)
    ...
    if (
      partsPercent >= 0.8 && // 80% of parts (high but not impossible)
  ...
  createStarDisplay() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const star = this.add
        .star(starX, starY, 5, 10, 20, earned ? 0xffd700 : 0x444444)
        .setOrigin(0.5)
        .setScrollFactor(0)
        .setDepth(201);
  ...
  updateUI() {
    ...
    this.scoreText.setText(
      `Score: ${window.GameUtils.formatScore(window.gameInstance.gameData.score)}`
    );
  ...
  createPowerUpQueueUI() {
    ...
    // Title text
    this.powerUpQueueTitle = this.add
      .text(uiX, uiY, 'Next Power-Up:', {
        fontSize: '14px',
        fill: '#ffffff',
        backgroundColor: '#000000',
        padding: { x: 8, y: 4 },
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const slotBg = this.add
        .rectangle(slotX, slotY, 60, 60, 0x333333, 0.8)
        .setScrollFactor(0)
        .setDepth(100)
    ...
    this.activePowerUpBg = this.add
      .rectangle(uiX, uiY + 100, 120, 50, 0x1a1a1a, 0.9)
      .setScrollFactor(0)
      .setDepth(100)
      .setStrokeStyle(2, 0x00ff00);

    this.activePowerUpText = this.add
      .text(uiX, uiY + 100, 'No Active\nPower-Up', {
        fontSize: '12px',
        fill: '#888888',
        align: 'center',
      })
  ...
  showPowerUpMessage(message, color = 0xffff00) {
    ...
    const messageText = this.add
      .text(player.sprite.x, player.sprite.y - 60, message, {
        fontSize: '16px',
