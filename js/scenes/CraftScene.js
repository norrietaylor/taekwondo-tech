...
class CraftScene extends Phaser.Scene {
  ...
  createLayout() {
    // Title
    this.add
      .text(this.cameras.main.centerX, 50, '🔧 ROBOT CRAFT WORKSHOP', {
        fontSize: '32px',
        fill: '#ffffff',
        fontWeight: 'bold',
      })
    ...
    // Inventory section
    this.add
      .text(this.inventoryArea.x, 100, 'Parts Inventory', {
        fontSize: '24px',
        fill: '#a8d5d1',
      })
      .setDepth(5);
    ...
    // Craft section
    this.add
      .text(this.craftArea.x, 100, 'Robot Assembly', {
        fontSize: '24px',
        fill: '#a8d5d1',
      })
      .setDepth(5);

    this.craftBg = this.add
      .rectangle(
  ...
  createPartItem(x, y, part, partType) {
    ...
    const label = this.add
      .text(0, 0, partType.charAt(0).toUpperCase(), {
        fontSize: '12px',
        fill: '#ffffff',
        fontWeight: 'bold',
      })
  ...
  updateRobotPreview() {
    ...
    this.robotPreview = this.add.container(this.craftArea.x + 200, this.craftArea.y + 150);
  ...
  createNavigation() {
    ...
    const outfitButton = this.add
      .text(50, 100, '👕 Change Outfit', {
        fontSize: '18px',
        fill: '#ffffff',
    ...
    // 🍌 Banana Mode button
    const bananaButton = this.add
      .text(50, 150, '🍌 Banana Mode', {
        fontSize: '18px',
        fill: '#000000',
    ...
    if (typeof PacManScene !== 'undefined') {
      const pacmanButton = this.add
        .text(250, 150, '🟡 Pac-Man Mode', {
          fontSize: '18px',
          fill: '#000000',
    ...
    if (window.gameInstance.gameData.currentLevel <= 6) {
      const continueButton = this.add
        .text(this.cameras.main.width - 50, 50, 'Continue to Next Level →', {
          fontSize: '18px',
          fill: '#ffffff',
  ...
  showOutfitSelection() {
    ...
    const overlay = this.add
      ...
      .setDepth(150)
      .setInteractive(); // Make overlay interactive to block clicks behind it
    ...
    if (totalPages > 1) {
      prevButton = this.add
        .text(this.cameras.main.centerX - 120, pagerY, '◀ PREV', {
          fontSize: '14px',
          fill: '#ffffff',
      ...
      nextButton = this.add
        .text(this.cameras.main.centerX + 120, pagerY, 'NEXT ▶', {
          fontSize: '14px',
          fill: '#ffffff',
          backgroundColor: '#444488',
          padding: { x: 12, y: 6 },
      ...
      pagerLabel = this.add
        .text(this.cameras.main.centerX, pagerY, '', {
          fontSize: '14px',
          fill: '#ffd700',
          fontWeight: 'bold',
        })
  ...
  _renderCostumeRow(
    ...
    // Dragon icon
    const icon = this.add
      .text(this.cameras.main.centerX - 253, y, costume.icon, {
        fontSize: '16px',
      })
      .setOrigin(0.5)
      .setDepth(153)
    ...
    if (isNewlyUnlocked) {
      const newBadge = this.add
        .text(this.cameras.main.centerX - 220, y - 20, 'NEW!', {
          fontSize: '10px',
          fill: '#ffff00',
    ...
    const nameText = this.add
      .text(this.cameras.main.centerX - 215, y - 12, costume.name, {
        fontSize: '14px',
    ...
    const descText = this.add
      .text(
        ...
        y + 2,
        costume.description.length > 45
          ? costume.description.substring(0, 42) + '...'
          : costume.description,
        {
          fontSize: '10px',
    ...
    const conditionText = this.add
      .text(this.cameras.main.centerX - 215, y + 15, this.getUnlockProgressText(costumeKey), {
        fontSize: '9px',
    ...
    if (!isUnlocked) {
      button = this.add
        .text(this.cameras.main.centerX + 260, y, '🔒 LOCKED', {
          fontSize: '11px',
          fill: '#666666',
      ...
      button = this.add
        .text(this.cameras.main.centerX + 260, y, '✓ EQUIPPED', {
          fontSize: '11px',
          fill: '#ffffff',
          backgroundColor: '#4a9eff',
          padding: { x: 8, y: 6 },
      ...
      button = this.add
        .text(this.cameras.main.centerX + 260, y, 'EQUIP', {
          fontSize: '11px',
          fill: '#ffffff',
      ...
      button.on('pointerdown', () => {
        const wasLegendary =
          window.gameInstance.getDragonCostume(window.gameInstance.gameData.outfits.current)
            .isLegendary || false;
        ...
        if (wasLegendary !== isLegendary) {
          if (isLegendary) {
            this.time.delayedCall(1800, () => {
              const notice = this.add.container(
                this.cameras.main.centerX,
                this.cameras.main.centerY
              );
              notice.setDepth(300);
    ...
    switch (costumeKey) {
      ...
      case 'legendary':
        ...
        return partTypes.every(
          (type) => gameData.robotParts[type] && gameData.robotParts[type].length > 0
        );
      default:
        return false;
    }
  ...
  showDragonEquippedNotification(costume) {
    ...
    // Text
    const text = this.add
      .text(20, 0, `${costume.name} Equipped!`, {
        fontSize: '22px',
        fill: '#ffffff',
        fontWeight: 'bold',
      })
  ...
  showBananaModeMenu() {
    ...
    const overlay = this.add
      .rectangle(this.cameras.main.centerX, this.cameras.main.centerY, 450, 380, 0x8b4513, 0.95)
      .setDepth(160);
    ...
    // Title
    const title = this.add
      .text(this.cameras.main.centerX, this.cameras.main.centerY - 150, '🍌 BANANA MODE 🍌', {
        fontSize: '36px',
        fill: '#FFE135',
        fontWeight: 'bold',
        stroke: '#5C4033',
    ...
    const survivalBtn = this.add
      .text(this.cameras.main.centerX, this.cameras.main.centerY - 20, '🏆 Endless Survival', {
        fontSize: '24px',
        fill: '#ffffff',
    ...
    const closeBtn = this.add
      .text(this.cameras.main.centerX, this.cameras.main.centerY + 130, 'Back', {
        fontSize: '20px',
        fill: '#ffffff',
        backgroundColor: '#ff6b6b',
        padding: { x: 30, y: 10 },
