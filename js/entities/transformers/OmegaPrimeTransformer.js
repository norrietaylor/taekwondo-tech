...
(function () {
  ...
  function getPalette(player) {
    const costume =
      player && typeof player.getDragonCostume === 'function' ? player.getDragonCostume() : null;
    const c = costume || {};
    return {
  ...
  function buildRobotVisuals(scene, player) {
    ...
    v.leftPauldronStripe = scene.add.rectangle(
      px - armX,
      py - 22 * S,
      20 * S,
      3 * S,
    ...
    v.rightPauldronStripe = scene.add.rectangle(
      px + armX,
      py - 22 * S,
      20 * S,
      3 * S,
  ...
  function positionRobot(parts, player) {
    ...
    if (parts.helmet) {
      parts.helmet.x = px;
      parts.helmet.y = R(py - 38 * S);
    }
    if (parts.chinGuard) {
      parts.chinGuard.x = px;
      parts.chinGuard.y = R(py - 28 * S);
    }
    if (parts.mouthGuard) {
      parts.mouthGuard.x = px;
      parts.mouthGuard.y = R(py - 28 * S);
    }
    if (parts.visor) {
      parts.visor.x = px;
      parts.visor.y = R(py - 40 * S);
    }
    if (parts.visorSheen) {
      parts.visorSheen.x = R(px - 6 * S);
      parts.visorSheen.y = R(py - 41 * S);
    }
    if (parts.crestFin) {
      parts.crestFin.x = px;
      parts.crestFin.y = Math.round(py - 60 * S);
    }
    if (parts.crestGem) {
      parts.crestGem.x = px;
      parts.crestGem.y = Math.round(py - 58 * S);
    }
    if (parts.earL) {
      parts.earL.x = R(px - 19 * S);
      parts.earL.y = R(py - 38 * S);
    }
    if (parts.earR) {
      parts.earR.x = R(px + 19 * S);
      parts.earR.y = R(py - 38 * S);
    }
    // Shoulders
    if (parts.leftPauldron) {
      parts.leftPauldron.x = px - armX;
      parts.leftPauldron.y = R(py - 18 * S);
    }
    if (parts.leftPauldronStripe) {
      parts.leftPauldronStripe.x = px - armX;
      parts.leftPauldronStripe.y = R(py - 22 * S);
    }
    if (parts.rightPauldron) {
      parts.rightPauldron.x = px + armX;
      parts.rightPauldron.y = R(py - 18 * S);
    }
    if (parts.rightPauldronStripe) {
      parts.rightPauldronStripe.x = px + armX;
      parts.rightPauldronStripe.y = R(py - 22 * S);
    }
    // Chest
    if (parts.chestPlate) {
      parts.chestPlate.x = px;
      parts.chestPlate.y = R(py - 2 * S);
    }
    if (parts.chevronL) {
      parts.chevronL.x = R(px - 6 * S);
      parts.chevronL.y = R(py - 4 * S);
    }
    if (parts.chevronR) {
      parts.chevronR.x = R(px + 6 * S);
      parts.chevronR.y = R(py - 4 * S);
    }
    if (parts.badge) {
      parts.badge.x = px;
      parts.badge.y = R(py + 4 * S);
    }
    // Arms / fists
    if (parts.leftGauntlet) {
      parts.leftGauntlet.x = px - armX;
      parts.leftGauntlet.y = R(py + 4 * S);
    }
    if (parts.leftFist) {
      parts.leftFist.x = px - armX;
      parts.leftFist.y = R(py + 16 * S);
    }
    if (parts.rightGauntlet) {
      parts.rightGauntlet.x = px + armX;
      parts.rightGauntlet.y = R(py + 4 * S);
    }
    if (parts.rightFist) {
      parts.rightFist.x = px + armX;
      parts.rightFist.y = R(py + 16 * S);
    }
    // Belt
    if (parts.belt) {
      parts.belt.x = px;
      parts.belt.y = R(py + 14 * S);
    }
    if (parts.buckle) {
      parts.buckle.x = px;
      parts.buckle.y = R(py + 14 * S);
    }
    if (parts.buckleOmega) {
      parts.buckleOmega.x = px;
      parts.buckleOmega.y = R(py + 14 * S);
    }
    // Legs / boots
    if (parts.leftKnee) {
      parts.leftKnee.x = px - legX;
      parts.leftKnee.y = R(py + 26 * S);
    }
    if (parts.rightKnee) {
      parts.rightKnee.x = px + legX;
      parts.rightKnee.y = R(py + 26 * S);
    }
    if (parts.leftBoot) {
      parts.leftBoot.x = px - legX;
      parts.leftBoot.y = R(py + 44 * S);
    }
    if (parts.leftBootStripe) {
      parts.leftBootStripe.x = px - legX;
      parts.leftBootStripe.y = R(py + 40 * S);
    }
    if (parts.rightBoot) {
      parts.rightBoot.x = px + legX;
      parts.rightBoot.y = R(py + 44 * S);
    }
    if (parts.rightBootStripe) {
      parts.rightBootStripe.x = px + legX;
      parts.rightBootStripe.y = R(py + 40 * S);
    }
  }
  ...
  function positionSnake(parts, player) {
    ...
    if (parts.hood) {
      parts.hood.x = px + dir * 6 * S;
      parts.hood.y = py + 4 * S;
    }
    if (parts.hoodMark) {
      parts.hoodMark.x = px + dir * 4 * S;
      parts.hoodMark.y = py - 2 * S;
    }
    if (parts.snout) {
      parts.snout.x = px + dir * 22 * S;
      parts.snout.y = py + 6 * S;
    }
    if (parts.leftEyeWhite) {
      parts.leftEyeWhite.x = px + dir * 10 * S;
      parts.leftEyeWhite.y = py - 1 * S;
    }
    if (parts.leftPupil) {
      parts.leftPupil.x = px + dir * 10 * S;
      parts.leftPupil.y = py - 1 * S;
    }
    if (parts.rightEyeWhite) {
      parts.rightEyeWhite.x = px + dir * 14 * S;
      parts.rightEyeWhite.y = py + 4 * S;
    }
    if (parts.rightPupil) {
      parts.rightPupil.x = px + dir * 14 * S;
      parts.rightPupil.y = py + 4 * S;
    }
    if (parts.fang1) {
      parts.fang1.x = px + dir * 22 * S;
      parts.fang1.y = py + 9 * S;
    }
    if (parts.fang2) {
      parts.fang2.x = px + dir * 26 * S;
      parts.fang2.y = py + 9 * S;
    }
    if (parts.tongueBase) {
      parts.tongueBase.x = px + dir * 32 * S;
      parts.tongueBase.y = py + 7 * S;
    }
  ...
  function transformEffect(scene, player, towardsSnake) {
    ...
    const text = scene.add
      .text(x, y - 60, towardsSnake ? '🐍 SERPENT MODE!' : '🤖 OMEGA PRIME!', {
        fontSize: '20px',
        fill: '#00e5ff',
        stroke: '#6e27c5',
        strokeThickness: 5,
  ...
  const omegaPrimeConfig = {
    ...
    buildVisuals(form, facingRight, player, transformer) {
      ...
      const parts =
        form === 'snake' ? buildSnakeVisuals(scene, player) : buildRobotVisuals(scene, player);
