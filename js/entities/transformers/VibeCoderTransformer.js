...
(function () {
  ...
  function positionRobot(parts, player) {
    ...
    if (parts.stripeA) {
      parts.stripeA.x = px - 5;
      parts.stripeA.y = py;
    }
    if (parts.stripeB) {
      parts.stripeB.x = px + 5;
      parts.stripeB.y = py;
    }
    if (parts.head) {
      parts.head.x = px;
    ...
    if (parts.leftArm) {
      parts.leftArm.x = px - 14;
      parts.leftArm.y = py - 2;
    }
    if (parts.rightArm) {
      parts.rightArm.x = px + 14;
      parts.rightArm.y = py - 2;
    }
    if (parts.leftLeg) {
      parts.leftLeg.x = px - 6;
      parts.leftLeg.y = py + 20;
    }
    if (parts.rightLeg) {
      parts.rightLeg.x = px + 6;
  ...
  function positionComputer(parts, player) {
    ...
    if (parts.keyboard) {
      parts.keyboard.x = px;
      parts.keyboard.y = py + 28;
    }
    if (parts.keyRow1) {
      parts.keyRow1.x = px;
      parts.keyRow1.y = py + 24;
    }
    if (parts.keyRow2) {
      parts.keyRow2.x = px;
      parts.keyRow2.y = py + 30;
    }
  }
  ...
  function transformEffect(scene, player, toComputer) {
    ...
    const label = scene.add
      .text(x, y - 50, toComputer ? '💻 COMPUTER MODE!' : '🤖 ROBOT MODE!', {
        fontSize: '16px',
        fill: '#00ff88',
        stroke: '#003311',
        strokeThickness: 3,
  ...
  function triggerCharm(transformer, player) {
    ...
    scene.enemies.children.entries.forEach((enemySprite) => {
      ...
      const enemy = typeof enemySprite.getData === 'function' ? enemySprite.getData('enemy') : null;
  ...
  const vibeCoderConfig = {
    ...
    onToggle(newForm, previousForm, player, transformer) {
      ...
      if (newForm === 'computer') {
        ...
        player.jumpPower =
          costume && typeof costume.computerJump === 'number' ? costume.computerJump : 0;
