...
(function () {
  ...
  function positionRobot(parts, player) {
    ...
    if (parts.stripeBlue) {
      parts.stripeBlue.x = px - 6;
      parts.stripeBlue.y = py;
    }
    if (parts.stripeViolet) {
      parts.stripeViolet.x = px;
      parts.stripeViolet.y = py;
    }
    if (parts.stripeRed) {
      parts.stripeRed.x = px + 6;
      parts.stripeRed.y = py;
    }
    if (parts.head) {
      parts.head.x = px;
      parts.head.y = py - 20;
    ...
    if (parts.leftArm) {
      parts.leftArm.x = px - 16;
      parts.leftArm.y = py - 2;
    }
    if (parts.rightArm) {
      parts.rightArm.x = px + 16;
      parts.rightArm.y = py - 2;
    }
    if (parts.leftLeg) {
      parts.leftLeg.x = px - 7;
      parts.leftLeg.y = py + 20;
    }
    if (parts.rightLeg) {
      parts.rightLeg.x = px + 7;
      parts.rightLeg.y = py + 20;
    }
    if (parts.leftFoot) {
      parts.leftFoot.x = px - 7;
      parts.leftFoot.y = py + 30;
    }
    if (parts.rightFoot) {
      parts.rightFoot.x = px + 7;
      parts.rightFoot.y = py + 30;
    }
  }
  ...
  function positionCar(parts, player) {
    ...
    if (parts.hood) {
      parts.hood.x = px + dir * 22;
      parts.hood.y = py - 2;
    }
    if (parts.roof) {
      parts.roof.x = px - dir * 4;
      parts.roof.y = py - 10;
    }
    if (parts.spoiler) {
      parts.spoiler.x = px - dir * 24;
      parts.spoiler.y = py - 6;
    }
    if (parts.wheel1) {
      parts.wheel1.x = px - dir * 20;
      parts.wheel1.y = py + 10;
    }
    if (parts.wheel2) {
      parts.wheel2.x = px + dir * 20;
      parts.wheel2.y = py + 10;
    }
    if (parts.liveryBlue) {
      parts.liveryBlue.x = px - 10;
      parts.liveryBlue.y = py - 4;
    }
    if (parts.liveryViolet) {
      parts.liveryViolet.x = px + 4;
      parts.liveryViolet.y = py - 4;
    }
    if (parts.liveryRed) {
      parts.liveryRed.x = px + 18;
      parts.liveryRed.y = py - 4;
    }
  }
