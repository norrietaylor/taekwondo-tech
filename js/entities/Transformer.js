...
class Transformer {
  constructor(player, config) {
    ...
    this.lastFacingRight =
      player && typeof player.facingRight === 'boolean' ? player.facingRight : true;
    this.visuals = [];
  ...
  rebuildVisualsIfNeeded(force) {
    ...
    const built =
      typeof this.config.buildVisuals === 'function'
  ...
  destroy() {
    ...
    if (typeof this.config.onDestroy === 'function') {
      try {
        this.config.onDestroy(this.player, this);
      } catch (e) {
        /* noop */
      }
    }
