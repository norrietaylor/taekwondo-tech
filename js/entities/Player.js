...
class Player {
    ...
    if (costume.hasWings) {
      ...
      for (let i = 0; i < segmentCount; i++) {
        ...
        const segment = this.scene.add.triangle(
          ...
          0,
          0,
          (-10 - i * 5) * sizeMultiplier, // top point
          (-15 - i * 5) * sizeMultiplier,
          5 * sizeMultiplier, // bottom left
          (-5 - i * 3) * sizeMultiplier,
          10 * sizeMultiplier, // bottom right
      ...
      for (let i = 0; i < segmentCount; i++) {
        ...
        const segment = this.scene.add.triangle(
          ...
          (-10 - i * 5) * sizeMultiplier, // top point
          (15 + i * 5) * sizeMultiplier,
          5 * sizeMultiplier, // bottom right
          (5 + i * 3) * sizeMultiplier,
          10 * sizeMultiplier, // bottom left
          i === segmentCount - 1 ? costume.wingTipColor : segmentColor,
          1.0 // Full opacity for visibility
        );
        segment.setStrokeStyle(2 * sizeMultiplier, 0x000000, 1.0); // Black border for visibility
  ...
  updateWings() {
    ...
    if (
      currentOutfit === 'grimlock' ||
      currentOutfit === 'bumblebee' ||
      currentOutfit === 'hotrod' ||
      currentOutfit === 'elita' ||
  ...
  update(time, delta) {
    ...
    if (!this.controls && this.controlsRetryAttempts < 10) {
      ...
      if (this.controlsRetryTimer > 100) {
        // Try every 100ms
        this.controlsRetryTimer = 0;
  ...
  handleCombat() {
    ...
    // Dragon costume with projectile - kick for elemental, punch for laser eyes
      ...
      if (this.powerUps.presentBomb) {
        ...
        if (
          this.controls.isKick() &&
          !this.previousInputs.kick &&
          this.specialAbilityCooldowns.presentBomb <= 0
        ) {
  ...
  handleSpecialAbilities() {
    ...
    if (
      this.controls.isKick() &&
      this.controls.isPunch() &&
      this.powerUps.fireBreath &&
      this.specialAbilityCooldowns.fireBreath <= 0
    ) {
  ...
  transformLaserToBoulder(laser, glow, direction) {
    ...
    const boulder = this.scene.add.polygon(
      x,
      y,
      [
        -boulderSize * 0.5,
  ...
  createStoneLaserHitEffect(x, y) {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const particle = this.scene.add.circle(x, y, 3, 0x778899, 0.9);
  ...
  createDinosaurVisuals() {
    ...
    for (let i = 0; i < 3; i++) {
      const tooth = this.scene.add.triangle(
        ...
        8, // bottom right
        -3,
        8, // bottom left - this makes it point DOWN
        0xffffff
      );
    ...
    this.grimlockTail = this.scene.add.polygon(
      px - dir * 20,
      py, // Moved closer to body center
      [
        dir * 5,
        -5, // Connect to body
        0,
  ...
  updateGrimlockVisualsIfNeeded() {
    ...
    const directionChanged =
      this.grimlockLastFacingRight !== undefined &&
  ...
  createGrimlockTransformEffect(towardsDino) {
    ...
    for (let i = 0; i < 16; i++) {
      ...
      const particle = this.scene.add.circle(x, y, 5 + Math.random() * 5, color, 0.9);
    ...
    const transformText = this.scene.add
      .text(x, y - 50, towardsDino ? '🦖 DINO MODE!' : '🤖 ROBOT MODE!', {
        fontSize: '20px',
        fill: '#ffd700',
        stroke: '#000000',
        strokeThickness: 4,
  ...
  updateBumblebeeVisuals() {
    if (this.bumblebeeVisuals && this.bumblebeeVisuals.length > 0) {
      this.bumblebeeVisuals.forEach((v) => {
  ...
  createBumblebeeTransformEffect(towardsCar) {
    ...
    const transformText = this.scene.add
      .text(x, y - 50, towardsCar ? '🚗 CAR MODE!' : '🤖 ROBOT MODE!', {
        fontSize: '20px',
        fill: '#ffd700',
        stroke: '#000000',
        strokeThickness: 4,
  ...
  updateBumblebeeVisualsIfNeeded() {
    ...
    const directionChanged =
      this.bumblebeeLastFacingRight !== undefined &&
      this.bumblebeeLastFacingRight !== this.facingRight;
    const needsRecreate =
      !this.bumblebeeVisuals ||
      this.bumblebeeVisuals.length === 0 ||
    ...
    if (this.bumblebeeVisuals && this.bumblebeeVisuals.length > 0) {
      if (this.bumblebeeForm === 'robot') {
        if (this.bumblebeeChest) {
          this.bumblebeeChest.x = px;
          this.bumblebeeChest.y = py;
        }
        if (this.bumblebeeHead) {
          this.bumblebeeHead.x = px;
          this.bumblebeeHead.y = py - 20;
        }
        if (this.bumblebeeVisor) {
          this.bumblebeeVisor.x = px;
          this.bumblebeeVisor.y = py - 20;
        }
        if (this.bumblebeeLeftArm) {
          this.bumblebeeLeftArm.x = px - 16;
          this.bumblebeeLeftArm.y = py - 2;
        }
        if (this.bumblebeeRightArm) {
          this.bumblebeeRightArm.x = px + 16;
          this.bumblebeeRightArm.y = py - 2;
        }
        if (this.bumblebeeLeftLeg) {
          this.bumblebeeLeftLeg.x = px - 7;
          this.bumblebeeLeftLeg.y = py + 20;
        }
        if (this.bumblebeeRightLeg) {
          this.bumblebeeRightLeg.x = px + 7;
          this.bumblebeeRightLeg.y = py + 20;
        }
        if (this.bumblebeeLeftFoot) {
          this.bumblebeeLeftFoot.x = px - 7;
          this.bumblebeeLeftFoot.y = py + 30;
        }
        if (this.bumblebeeRightFoot) {
          this.bumblebeeRightFoot.x = px + 7;
          this.bumblebeeRightFoot.y = py + 30;
        }
      } else {
        if (this.bumblebeeBody) {
          this.bumblebeeBody.x = px;
          this.bumblebeeBody.y = py;
        }
        if (this.bumblebeeHood) {
          this.bumblebeeHood.x = px + dir * 20;
          this.bumblebeeHood.y = py - 2;
        }
        if (this.bumblebeeRear) {
          this.bumblebeeRear.x = px - dir * 20;
          this.bumblebeeRear.y = py - 2;
        }
        if (this.bumblebeeWheel1) {
          this.bumblebeeWheel1.x = px - dir * 18;
          this.bumblebeeWheel1.y = py + 10;
        }
        if (this.bumblebeeWheel2) {
          this.bumblebeeWheel2.x = px + dir * 18;
          this.bumblebeeWheel2.y = py + 10;
        }
        if (this.bumblebeeStripe) {
          this.bumblebeeStripe.x = px;
          this.bumblebeeStripe.y = py - 6;
        }
      }
    }
  }
  ...
  createHotrodLaughterEffect() {
    ...
    const laughText = this.scene.add
      .text(x, y - 45, 'HA HA HA!', {
        fontSize: '24px',
        fill: '#ffd700',
        stroke: '#e63946',
        strokeThickness: 4,
  ...
  createHotrodTransformEffect(towardsCar) {
    ...
    const transformText = this.scene.add
      .text(x, y - 50, towardsCar ? '🏎️ CAR MODE!' : '🤖 ROBOT MODE!', {
        fontSize: '20px',
        fill: '#e63946',
        stroke: '#1a1a1a',
        strokeThickness: 4,
  ...
  updateHotrodVisualsIfNeeded() {
    ...
    const directionChanged =
      this.hotrodLastFacingRight !== undefined && this.hotrodLastFacingRight !== this.facingRight;
    const needsRecreate =
      !this.hotrodVisuals ||
      this.hotrodVisuals.length === 0 ||
    ...
    if (this.hotrodVisuals && this.hotrodVisuals.length > 0) {
      if (this.hotrodForm === 'robot') {
        if (this.hotrodChest) {
          this.hotrodChest.x = px;
          this.hotrodChest.y = py;
        }
        if (this.hotrodHead) {
          this.hotrodHead.x = px;
          this.hotrodHead.y = py - 20;
        }
        if (this.hotrodVisor) {
          this.hotrodVisor.x = px;
          this.hotrodVisor.y = py - 20;
        }
        if (this.hotrodLeftArm) {
          this.hotrodLeftArm.x = px - 16;
          this.hotrodLeftArm.y = py - 2;
        }
        if (this.hotrodRightArm) {
          this.hotrodRightArm.x = px + 16;
          this.hotrodRightArm.y = py - 2;
        }
        if (this.hotrodLeftLeg) {
          this.hotrodLeftLeg.x = px - 7;
          this.hotrodLeftLeg.y = py + 20;
        }
        if (this.hotrodRightLeg) {
          this.hotrodRightLeg.x = px + 7;
          this.hotrodRightLeg.y = py + 20;
        }
        if (this.hotrodLeftFoot) {
          this.hotrodLeftFoot.x = px - 7;
          this.hotrodLeftFoot.y = py + 30;
        }
        if (this.hotrodRightFoot) {
          this.hotrodRightFoot.x = px + 7;
          this.hotrodRightFoot.y = py + 30;
        }
      } else {
        if (this.hotrodBody) {
          this.hotrodBody.x = px;
          this.hotrodBody.y = py;
        }
        if (this.hotrodHood) {
          this.hotrodHood.x = px + dir * 22;
          this.hotrodHood.y = py - 4;
        }
        if (this.hotrodRear) {
          this.hotrodRear.x = px - dir * 22;
          this.hotrodRear.y = py - 2;
        }
        if (this.hotrodWheel1) {
          this.hotrodWheel1.x = px - dir * 20;
          this.hotrodWheel1.y = py + 10;
        }
        if (this.hotrodWheel2) {
          this.hotrodWheel2.x = px + dir * 20;
          this.hotrodWheel2.y = py + 10;
        }
        if (this.hotrodStripe) {
          this.hotrodStripe.x = px;
          this.hotrodStripe.y = py - 4;
        }
      }
    }
  }
  ...
  createElitaTransformEffect(towardsBike) {
    ...
    const transformText = this.scene.add
      .text(x, y - 50, towardsBike ? '🏍️ BIKE MODE!' : '🤖 ROBOT MODE!', {
        fontSize: '20px',
        fill: '#e91e8c',
        stroke: '#4a148c',
        strokeThickness: 4,
  ...
  updateElitaVisualsIfNeeded() {
    ...
    const directionChanged =
      this.elitaLastFacingRight !== undefined && this.elitaLastFacingRight !== this.facingRight;
    const needsRecreate =
      !this.elitaVisuals ||
      this.elitaVisuals.length === 0 ||
    ...
    if (this.elitaVisuals && this.elitaVisuals.length > 0) {
      if (this.elitaForm === 'robot') {
        if (this.elitaChest) {
          this.elitaChest.x = px;
          this.elitaChest.y = py;
        }
        if (this.elitaHead) {
          this.elitaHead.x = px;
          this.elitaHead.y = py - 20;
        }
        if (this.elitaVisor) {
          this.elitaVisor.x = px;
          this.elitaVisor.y = py - 20;
        }
        if (this.elitaLeftArm) {
          this.elitaLeftArm.x = px - 16;
          this.elitaLeftArm.y = py - 2;
        }
        if (this.elitaRightArm) {
          this.elitaRightArm.x = px + 16;
          this.elitaRightArm.y = py - 2;
        }
        if (this.elitaLeftLeg) {
          this.elitaLeftLeg.x = px - 7;
          this.elitaLeftLeg.y = py + 20;
        }
        if (this.elitaRightLeg) {
          this.elitaRightLeg.x = px + 7;
          this.elitaRightLeg.y = py + 20;
        }
        if (this.elitaLeftFoot) {
          this.elitaLeftFoot.x = px - 7;
          this.elitaLeftFoot.y = py + 30;
        }
        if (this.elitaRightFoot) {
          this.elitaRightFoot.x = px + 7;
          this.elitaRightFoot.y = py + 30;
        }
      } else {
        if (this.elitaBody) {
          this.elitaBody.x = px;
        ...
        if (this.elitaSeat) {
          this.elitaSeat.x = px;
          this.elitaSeat.y = py - 4;
        }
        if (this.elitaWheel1) {
          this.elitaWheel1.x = px - dir * 22;
          this.elitaWheel1.y = py + 8;
        }
        if (this.elitaWheel2) {
          this.elitaWheel2.x = px + dir * 22;
          this.elitaWheel2.y = py + 8;
        }
        if (this.elitaStripe) {
          this.elitaStripe.x = px;
          this.elitaStripe.y = py - 2;
        }
      }
    }
  }
  ...
  updateBmwBouncerVisuals() {
    if (this.bmwBouncerVisuals && this.bmwBouncerVisuals.length > 0) {
      this.bmwBouncerVisuals.forEach((v) => {
        if (v && v.destroy) v.destroy();
      });
    }
    this.bmwBouncerVisuals = [];
  ...
  createBmwBouncerTransformEffect(towardsCar) {
    ...
    const transformText = this.scene.add
      .text(x, y - 50, towardsCar ? '🏁 CSL 3.0 M MODE!' : '🤖 ROBOT MODE!', {
        fontSize: '18px',
        fill: '#ffffff',
        stroke: '#0066b2',
        strokeThickness: 4,
  ...
  updatePortalbotVisuals() {
    if (this.portalbotVisuals && this.portalbotVisuals.length > 0) {
      this.portalbotVisuals.forEach((v) => {
        if (v && v.destroy) v.destroy();
      });
    }
    this.portalbotVisuals = [];
  ...
  createPortalbotTransformEffect(towardsDragon) {
    ...
    const transformText = this.scene.add
      .text(x, y - 50, towardsDragon ? '🐉 DRAGON MODE!' : '🤖 ROBOT MODE!', {
        fontSize: '20px',
        fill: '#00e5ff',
        stroke: '#7b1fa2',
        strokeThickness: 4,
  ...
  updateBmwBouncerVisualsIfNeeded() {
    ...
    const directionChanged =
      this.bmwBouncerLastFacingRight !== undefined &&
      this.bmwBouncerLastFacingRight !== this.facingRight;
    const needsRecreate =
      !this.bmwBouncerVisuals ||
      this.bmwBouncerVisuals.length === 0 ||
    ...
    if (this.bmwBouncerVisuals && this.bmwBouncerVisuals.length > 0) {
      if (this.bmwBouncerForm === 'robot') {
        if (this.bmwBouncerChest) {
          this.bmwBouncerChest.x = px;
          this.bmwBouncerChest.y = py;
        }
        if (this.bmwBouncerStripeBlue) {
          this.bmwBouncerStripeBlue.x = px - 6;
          this.bmwBouncerStripeBlue.y = py;
        }
        if (this.bmwBouncerStripeViolet) {
          this.bmwBouncerStripeViolet.x = px;
          this.bmwBouncerStripeViolet.y = py;
        }
        if (this.bmwBouncerStripeRed) {
          this.bmwBouncerStripeRed.x = px + 6;
          this.bmwBouncerStripeRed.y = py;
        }
        if (this.bmwBouncerHead) {
          this.bmwBouncerHead.x = px;
          this.bmwBouncerHead.y = py - 20;
        }
        if (this.bmwBouncerVisor) {
          this.bmwBouncerVisor.x = px;
          this.bmwBouncerVisor.y = py - 20;
        }
        if (this.bmwBouncerLeftArm) {
          this.bmwBouncerLeftArm.x = px - 16;
          this.bmwBouncerLeftArm.y = py - 2;
        }
        if (this.bmwBouncerRightArm) {
          this.bmwBouncerRightArm.x = px + 16;
          this.bmwBouncerRightArm.y = py - 2;
        }
        if (this.bmwBouncerLeftLeg) {
          this.bmwBouncerLeftLeg.x = px - 7;
          this.bmwBouncerLeftLeg.y = py + 20;
        }
        if (this.bmwBouncerRightLeg) {
          this.bmwBouncerRightLeg.x = px + 7;
          this.bmwBouncerRightLeg.y = py + 20;
        }
        if (this.bmwBouncerLeftFoot) {
          this.bmwBouncerLeftFoot.x = px - 7;
          this.bmwBouncerLeftFoot.y = py + 30;
        }
        if (this.bmwBouncerRightFoot) {
          this.bmwBouncerRightFoot.x = px + 7;
          this.bmwBouncerRightFoot.y = py + 30;
        }
      } else {
        if (this.bmwBouncerBody) {
          this.bmwBouncerBody.x = px;
          this.bmwBouncerBody.y = py;
        }
        if (this.bmwBouncerHood) {
          this.bmwBouncerHood.x = px + dir * 22;
          this.bmwBouncerHood.y = py - 2;
        }
        if (this.bmwBouncerRoof) {
          this.bmwBouncerRoof.x = px - dir * 4;
          this.bmwBouncerRoof.y = py - 10;
        }
        if (this.bmwBouncerSpoiler) {
          this.bmwBouncerSpoiler.x = px - dir * 24;
          this.bmwBouncerSpoiler.y = py - 6;
        }
        if (this.bmwBouncerWheel1) {
          this.bmwBouncerWheel1.x = px - dir * 20;
          this.bmwBouncerWheel1.y = py + 10;
        }
        if (this.bmwBouncerWheel2) {
          this.bmwBouncerWheel2.x = px + dir * 20;
          this.bmwBouncerWheel2.y = py + 10;
        }
        if (this.bmwBouncerLiveryBlue) {
          this.bmwBouncerLiveryBlue.x = px - 10;
          this.bmwBouncerLiveryBlue.y = py - 4;
        }
        if (this.bmwBouncerLiveryViolet) {
          this.bmwBouncerLiveryViolet.x = px + 4;
          this.bmwBouncerLiveryViolet.y = py - 4;
        }
        if (this.bmwBouncerLiveryRed) {
          this.bmwBouncerLiveryRed.x = px + 18;
          this.bmwBouncerLiveryRed.y = py - 4;
        }
      }
    }
  }
  ...
  updatePortalbotVisualsIfNeeded() {
    ...
    const directionChanged =
      this.portalbotLastFacingRight !== undefined &&
      this.portalbotLastFacingRight !== this.facingRight;
    const needsRecreate =
      !this.portalbotVisuals ||
      this.portalbotVisuals.length === 0 ||
    ...
    if (this.portalbotVisuals && this.portalbotVisuals.length > 0) {
      if (this.portalbotForm === 'robot') {
        if (this.portalbotChest) {
          this.portalbotChest.x = px;
          this.portalbotChest.y = py;
        }
        if (this.portalbotHead) {
          this.portalbotHead.x = px;
          this.portalbotHead.y = py - 20;
        }
        if (this.portalbotVisor) {
          this.portalbotVisor.x = px;
          this.portalbotVisor.y = py - 20;
        }
        if (this.portalbotCore) {
          this.portalbotCore.x = px;
          this.portalbotCore.y = py;
        }
        if (this.portalbotLeftArm) {
          this.portalbotLeftArm.x = px - 16;
          this.portalbotLeftArm.y = py - 2;
        }
        if (this.portalbotRightArm) {
          this.portalbotRightArm.x = px + 16;
          this.portalbotRightArm.y = py - 2;
        }
        if (this.portalbotLeftLeg) {
          this.portalbotLeftLeg.x = px - 7;
          this.portalbotLeftLeg.y = py + 20;
        }
        if (this.portalbotRightLeg) {
          this.portalbotRightLeg.x = px + 7;
          this.portalbotRightLeg.y = py + 20;
        }
        if (this.portalbotLeftFoot) {
          this.portalbotLeftFoot.x = px - 7;
          this.portalbotLeftFoot.y = py + 30;
        }
        if (this.portalbotRightFoot) {
          this.portalbotRightFoot.x = px + 7;
          this.portalbotRightFoot.y = py + 30;
        }
      } else {
        // Dragon form position updates
        if (this.portalbotBody) {
          this.portalbotBody.x = px;
          this.portalbotBody.y = py;
        }
        if (this.portalbotDragonHead) {
          this.portalbotDragonHead.x = px + dir * 20;
          this.portalbotDragonHead.y = py - 10;
        }
        if (this.portalbotEye) {
          this.portalbotEye.x = px + dir * 24;
          this.portalbotEye.y = py - 14;
        }
        if (this.portalbotSnout) {
          this.portalbotSnout.x = px + dir * 32;
          this.portalbotSnout.y = py - 8;
        }
        if (this.portalbotTail) {
          this.portalbotTail.x = px - dir * 24;
          this.portalbotTail.y = py + 2;
        }
        if (this.portalbotTailSwirl) {
          this.portalbotTailSwirl.x = px - dir * 34;
          this.portalbotTailSwirl.y = py + 2;
        }
        if (this.portalbotWing1) {
          this.portalbotWing1.x = px - dir * 4;
          this.portalbotWing1.y = py - 18;
        }
        if (this.portalbotWing2) {
          this.portalbotWing2.x = px - dir * 8;
          this.portalbotWing2.y = py - 14;
        }
        if (this.portalbotDragonLeftLeg) {
          this.portalbotDragonLeftLeg.x = px - 8;
          this.portalbotDragonLeftLeg.y = py + 16;
        }
        if (this.portalbotDragonRightLeg) {
          this.portalbotDragonRightLeg.x = px + 8;
          this.portalbotDragonRightLeg.y = py + 16;
        }
        if (this.portalbotLeftClaw) {
          this.portalbotLeftClaw.x = px - 8;
          this.portalbotLeftClaw.y = py + 24;
        }
        if (this.portalbotRightClaw) {
          this.portalbotRightClaw.x = px + 8;
          this.portalbotRightClaw.y = py + 24;
        }
      }
    }
  }
  ...
  performBmwBouncerBounceSlam() {
    ...
    const slamText = this.scene.add
      .text(px, py - 50, 'BOUNCE SLAM!', {
        fontSize: '22px',
        fill: '#ffffff',
        stroke: '#e22400',
        strokeThickness: 4,
  ...
  createDuckLaserBeam(startX, startY, direction) {
    ...
    // Add duck emoji glow
    const duckEmoji = this.scene.add
  ...
  transformToDuck(enemy, enemySprite) {
    ...
    const duck = this.scene.add
      .text(duckX, duckY, '🦆', { fontSize: '40px' })
      .setOrigin(0.5)
    ...
    for (let i = 0; i < 8; i++) {
      const feather = this.scene.add
        .text(duckX, duckY, '🪶', { fontSize: '16px' })
        .setOrigin(0.5)
        .setDepth(100);
    ...
    // Add "QUACK!" text
    const quackText = this.scene.add
      .text(duckX, duckY - 40, 'QUACK! 🦆', {
        fontSize: '18px',
        fill: '#ffd700',
        stroke: '#000000',
        strokeThickness: 3,
  ...
  updateDuckedEnemies(_delta) {
    ...
    for (let i = this.duckedEnemies.length - 1; i >= 0; i--) {
      ...
      if (now >= ducked.restoreTime) {
        ...
        if (
          ducked.duckSprite &&
          ducked.duckSprite.active &&
          ducked.sprite &&
          ducked.sprite.active
        ) {
  ...
  restoreFromDuck(ducked) {
    ...
    const warningText = this.scene.add
      .text(sprite.x, sprite.y - 40, '😠 Back to normal!', {
        fontSize: '14px',
        fill: '#ff4444',
        stroke: '#000000',
        strokeThickness: 2,
  ...
  createDogLaserBeam(startX, startY, direction) {
    ...
    const laserWidth = 100;
    const laserHeight = 16;
    const laser = this.scene.add.rectangle(startX, startY, laserWidth, laserHeight, 0xffd700, 0.9);
    laser.setStrokeStyle(3, 0x000000);
    laser.setDepth(100);
    const dogEmoji = this.scene.add
      .text(startX + direction * 50, startY, '🐕', { fontSize: '24px' })
      .setOrigin(0.5)
      .setDepth(101);
  ...
  transformToDog(enemy, enemySprite) {
    ...
    const woofText = this.scene.add
      .text(dogX, dogY - 40, 'WOOF! 🐕', {
        fontSize: '18px',
        fill: '#ffd700',
        stroke: '#000000',
        strokeThickness: 3,
  ...
  shootDragonProjectile() {
    ...
    switch (projectileType) {
      ...
      case 'elitaDogBullet':
        ...
        projectile = this.scene.add.ellipse(
          startX,
          startY,
          costume.projectileSize * 1.2,
          costume.projectileSize,
          costume.projectileColor,
          0.95
        );
  ...
  createIceProjectile(x, y, costume) {
    ...
    const projectile = this.scene.add.polygon(
      x,
      y,
      [
        0,
  ...
  createGrimlockBreathProjectile(x, y, costume) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const bolt = this.scene.add
        .text(Math.cos(angle) * size * 0.9, Math.sin(angle) * size * 0.9, '⚡', {
          fontSize: '14px',
        })
        .setOrigin(0.5);
      container.add(bolt);
    ...
    const trailTimer = this.scene.time.addEvent({
      ...
      callback: () => {
        ...
        if (isFireParticle) {
          ...
          const lightningParticle = this.scene.add
            .text(
  ...
  placePortal(x, y) {
    ...
    // Portal oval frame
    const frame = this.scene.add.ellipse(
      x,
      y - portalHeight / 2,
      portalWidth,
      portalHeight,
    ...
    const icon = this.scene.add
      .text(x, y - portalHeight / 2, '🌀', { fontSize: '24px' })
      .setOrigin(0.5)
      .setDepth(47);
  ...
  checkProjectileBananaCollision(projectile, _projectileIndex) {
    ...
    bananas.forEach((banana) => {
      ...
      if (distance < 35) {
        ...
        const velocityX = projectile.sprite.body
          ? projectile.sprite.body.velocity.x
          : this.facingRight
            ? 1
            : -1;
  ...
  createLightningArc(x1, y1, x2, y2) {
    ...
    points.forEach((point, idx) => {
      ...
      const bolt = this.scene.add.line(0, 0, prevPoint.x, prevPoint.y, point.x, point.y, 0xffff00);
      bolt.setLineWidth(3);
      bolt.setDepth(70);
  ...
  checkAttackHit(attackType) {
    ...
    this.scene.add.circle(attackX, attackY, 8, effectColor, 0.7).setDepth(10).setScale(0.5);
  ...
  createDoubleJumpEffect() {
    ...
    const costume = this.getDragonCostume();
    const effect1 = this.scene.add.circle(
      this.sprite.x,
      this.sprite.y,
      12,
      costume.effectColor,
      ...
      if (enemy.active) {
        ...
        if (distance < radius) {
          if (enemy.getData && enemy.getData('enemy')) {
            ...
            enemy.body.setVelocity(Math.cos(knockbackAngle) * 300, Math.sin(knockbackAngle) * 200);
          }
        }
      }
    });
  ...
  spawnDragonAlly(x, y) {
    ...
    // Dragon tail
    const tail = this.scene.add.polygon(
      -25,
      5,
      [
        [0, 0],
    ...
    if (this.scene.enemies) {
      this.scene.enemies.children.entries.forEach((enemy) => {
        if (enemy.active && enemy.getData('enemy') && enemy.getData('enemy').health > 0) {
          const distance = Phaser.Math.Distance.Between(dragon.x, dragon.y, enemy.x, enemy.y);
  ...
  createDragonAllyHitEffect(x, y, color) {
    ...
    for (let i = 0; i < 6; i++) {
      const angle = (i / 6) * Math.PI * 2;
      const particle = this.scene.add.circle(x, y, 4 + Math.random() * 3, color, 0.8);
  ...
  createDragonAura() {
    ...
    const aura = this.scene.add.circle(this.sprite.x, this.sprite.y, 40, costume.effectColor, 0.2);
