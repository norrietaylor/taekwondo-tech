...
class Player {
    ...
    updateWings() {
        ...
        if (currentOutfit === 'grimlock' || currentOutfit === 'bumblebee' || currentOutfit === 'hotrod' || currentOutfit === 'elita' || currentOutfit === 'bmwBouncer' || currentOutfit === 'portalbot' || currentOutfit === 'omegaPrime' || !costume.hasWings) {
            this.leftWing.setVisible(false);
            this.rightWing.setVisible(false);
            return;
        }
    ...
    handleCombat() {
        ...
        const omegaSnakeForm = (costume.isOmegaPrime
            && this.transformer
    ...
    handleOmegaPrimeAltTheme() {
        ...
        if (pressed && !this.previousInputs.omegaAltTheme) {
            ...
            const swapKeys = [
                'primaryColor', 'secondaryColor', 'beltColor', 'effectColor',
                'wingColor', 'wingTipColor', 'projectileColor',
                'projectileSecondaryColor', 'fireballColors',
                'robotColors'
            ];
            ...
            const label = this.scene.add.text(this.sprite.x, this.sprite.y - 80,
                '🎨 ' + (costume._currentThemeName || 'Theme swap'),
                { fontSize: '18px', fill: '#ffffff', stroke: '#000000', strokeThickness: 4, fontWeight: 'bold' }
            ).setOrigin(0.5).setDepth(150);
            this.scene.tweens.add({
                targets: label,
                y: this.sprite.y - 120, alpha: 0,
                duration: 1100,
    ...
    handleOmegaBlast() {
        ...
        const currentOutfit = window.gameInstance?.gameData?.outfits?.current || 'default';
        if (currentOutfit !== 'omegaPrime') return;
        const pressed = (typeof this.controls.isOmegaBlast === 'function')
            ? this.controls.isOmegaBlast() : false;
    ...
    performOmegaBlast() {
        if (this.omegaBlastCooldown > 0) {
            console.log('🌀 O-MEGA BLAST on cooldown!',
                Math.ceil(this.omegaBlastCooldown / 1000), 's remaining');
            return;
        }
        ...
        const label = this.scene.add.text(this.sprite.x, this.sprite.y - 70, 'O-MEGA BLAST!', {
            fontSize: '22px',
    ...
    createOmegaBlastLaser(startX, startY, angle, theme) {
        ...
        const laser = this.scene.add.rectangle(startX, startY, laserWidth, laserHeight, theme.primary, 0.95);
        laser.setStrokeStyle(2, theme.secondary);
        laser.setRotation(angle); // align the beam with its travel direction
        laser.setDepth(100);

        const glow = this.scene.add.rectangle(startX, startY, laserWidth + 14, laserHeight + 12, theme.secondary, 0.35);
        ...
        const trail = this.scene.time.addEvent({
            delay: 45, repeat: 14,
            callback: () => {
                if (!laser.active) { trail.remove(); return; }
                ...
                const p = this.scene.add.circle(
                    ...
                    3 + Math.random() * 4,
                    cols[Math.floor(Math.random() * cols.length)], 0.7);
                p.setDepth(98);
                this.scene.tweens.add({
                    targets: p, alpha: 0, scale: 0.3, duration: 280,
        ...
        this.scene.time.delayedCall(laserDistance / laserSpeed * 1000, () => {
            if (!transformed && laser.active) {
                transformed = true;
                const endX = laser.x, endY = laser.y;
                laser.destroy();
                glow.destroy();
    ...
    createOmegaVortex(x, y, theme) {
        ...
        for (let i = 0; i < armCount; i++) {
            ...
            const arm = this.scene.add.triangle(
                ...
                0, -6, 46, 0, 0, 6,
                (i % 2 === 0) ? theme.primary : theme.secondary, 0.9);
            arm.setRotation(ang);
            vortex.add(arm);
        }
        ...
        const spiral = this.scene.time.addEvent({
            delay: 40, repeat: 14,
            callback: () => {
                if (!vortex.active) { spiral.remove(); return; }
                const a = Math.random() * Math.PI * 2;
                const cols = [theme.primary, theme.secondary, theme.accent];
                const p = this.scene.add.circle(
                    x + Math.cos(a) * 55, y + Math.sin(a) * 55,
                    3 + Math.random() * 3,
                    cols[Math.floor(Math.random() * cols.length)], 0.85);
                p.setDepth(107);
                this.scene.tweens.add({
                    targets: p, x: x, y: y, alpha: 0, scale: 0.2,
    ...
    createOmegaVortexExplosion(x, y, theme) {
        ...
        flash.setDepth(112);
        this.scene.tweens.add({
            targets: flash, scale: radius / 16, alpha: 0, duration: 320,
        ...
        for (let i = 0; i < 18; i++) {
            ...
            const shard = this.scene.add.triangle(
                ...
                0, -8, 6, 6, -6, 6,
                cols[Math.floor(Math.random() * cols.length)], 0.95);
    ...
    createOmegaBlastChargeEffect(x, y, theme) {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            const cols = [theme.primary, theme.secondary, theme.accent];
            const p = this.scene.add.circle(
                x + Math.cos(ang) * 55, y + Math.sin(ang) * 55,
                5, cols[i % cols.length], 0.85);
            p.setDepth(103);
            this.scene.tweens.add({
                targets: p, x: x, y: y, alpha: 0, scale: 0.4,
                duration: 220, ease: 'Power2', onComplete: () => p.destroy()
            });
        }
    ...
    shootSnakeFireLaser() {
        ...
        const flame = this.scene.add.rectangle(startX, startY, laserLength + 14, laserHeight + 8, 0x6e27c5, 0.55);
        flame.setDepth(101);
        ...
        for (let i = 0; i < 3; i++) {
            const spark = this.scene.add.circle(startX - dir * (8 + i * 6), startY, 3 + i, sparkColors[i], 0.85);
            spark.setDepth(100);
            this.scene.tweens.add({
                targets: spark,
                alpha: 0,
    ...
    storePreviousInputs() {
        ...
        this.previousInputs.omegaBlast = (typeof this.controls.isOmegaBlast === 'function')
            ? this.controls.isOmegaBlast() : false;
