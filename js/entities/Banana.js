  ...
  update(time, delta) {
    ...
    if (this.body && this.body.velocity) {
      const speed = Math.sqrt(
        this.body.velocity.x * this.body.velocity.x + this.body.velocity.y * this.body.velocity.y
      );
...
class BananaManager {
  ...
  showFoodTypeIndicator(foodType) {
    ...
    this.foodIndicator = this.scene.add
      .container(this.scene.cameras.main.width - 80, 100)
      .setScrollFactor(0)
      .setDepth(150);
    ...
    // Emoji and text
    const text = this.scene.add
      .text(0, 0, `${config.emoji} ${config.name}`, {
        fontSize: '16px',
        fill: '#ffffff',
        fontWeight: 'bold',
        stroke: '#000000',
  ...
  revertTobanana() {
    ...
    // Show revert message
    const msg = this.scene.add
      .text(this.scene.cameras.main.centerX, 100, '🍌 Back to Bananas!', {
        fontSize: '24px',
        fill: '#FFE135',
        fontWeight: 'bold',
        stroke: '#8B4513',
  ...
  spawnBanana() {
    ...
    console.log(
      `${foodConfig.emoji} Spawned ${foodConfig.name} #${this.bananasSpawned} (next in ${this.currentSpawnRate}ms)`
    );
  }
