...
class Controls {
  ...
  setupMobileControls() {
    ...
    const forceTouch =
      urlParams.get('touch') === 'true' || localStorage.getItem('forceMobileControls') === 'true';
  ...
  isDown() {
    return (
      this.keys['ArrowDown'] || this.keys['KeyS'] || (this.isMobile && this.mobile.joystick.y > 0.3)
    );
  }
  ...
  isKick() {
    return this.keys['KeyX'] || (this.isMobile && this.mobile.buttons.kick);
  }

  isPunch() {
    return this.keys['KeyZ'] || (this.isMobile && this.mobile.buttons.punch);
  }
  ...
  isActivate() {
    return (
      this.keys['KeyE'] || this.keys['KeyQ'] || (this.isMobile && this.mobile.buttons.activate)
    );
  }
